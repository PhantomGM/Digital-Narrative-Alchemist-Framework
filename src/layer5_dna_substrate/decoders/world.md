...
1. The DNA code is for **internal processing only**. **DO NOT** display or reference the DNA string, its block names (`ENV{}`, `SOC{}`, `CON{}` …), its axis labels, or its numeric values anywhere in the output.
2. This applies to parentheticals especially. Writing `(T:10, but TEC:2, TM:2, KNG:1)` after a sentence is exactly the failure — it turns a setting description into a decoded readout. State the *consequence* instead: a world whose ambitions vastly outrun what it can actually build.
3. Everything the DNA encodes must reach the page as setting: geography, history, pressure, and the shape of the trouble. Never as scores.
